...
import type { Identity, Role } from "../../generated/prisma/client";
...
export async function createIdentity(
	...
	return prisma.identity.create({
		data: {
			...
			// status and failedLoginAttempts use schema defaults
		},
	});
}
...
// Lists all non-deleted identities for a given tenant.
// Uses @@index([tenantId]).
export async function listIdentitiesByTenant(
	tenantId: string,
): Promise<Identity[]> {
...
// Lists non-deleted identities filtered by status within a tenant.
// Uses @@index([tenantId, status]).
...
// Lists non-deleted identities filtered by kind within a tenant.
// Uses @@index([tenantId, kind]).
export async function listIdentitiesByKind(
	tenantId: string,
	kind: Identity["kind"],
): Promise<Identity[]> {
	return prisma.identity.findMany({
